    ...
    VkDescriptorType type)
{
    ...
    w.dstSet = VK_NULL_HANDLE; // filled in update_set
    w.dstBinding = binding;
    w.dstArrayElement = arrayElement; // << important
    w.descriptorCount = 1;
...
void DescriptorWriter::write_buffer(int binding, VkBuffer buffer, size_t size, size_t offset, VkDescriptorType type) {
    ...
    write.dstBinding = binding; // (layout = ...)
    write.dstSet = VK_NULL_HANDLE; //left empty for now until we need to write it
    write.descriptorCount = 1;
    write.descriptorType = type;
    write.pBufferInfo = &info; // deque is guaranteed to keep these pointers valid, i.e. I guess it doesn't relocate?
...
VkDescriptorSet DescriptorAllocatorGrowable::allocate(VkDevice device, VkDescriptorSetLayout layout, void* pNext) {
    ...
    if (result == VK_ERROR_OUT_OF_POOL_MEMORY || result == VK_ERROR_FRAGMENTED_POOL) {
        ...
        VK_CHECK(vkAllocateDescriptorSets(device, &allocInfo, &ds)); // TODO: I feel like there's no guarantee this is ready either tho
...
VkDescriptorPool DescriptorAllocatorGrowable::get_pool(VkDevice device) {
    ...
    if (readyPools.size() != 0) {
        ...
        if (setsPerPool > 4092) { // this max limit can be modified
            setsPerPool = 4092;
        }
    }
...
// pool sizes give how many individual bindings of a given type are owned
