#version 450
#extension GL_GOOGLE_include_directive : require
#extension GL_EXT_buffer_reference : require

#include "input_structures_pbr.glsl"

layout (location = 0) out vec3 vWorldPos;
layout (location = 1) out vec3 vWorldNormal;
layout (location = 2) out vec2 vUV;
layout (location = 3) out vec3 vVertexColor;

struct Vertex {
    vec3 position;
    float uv_x;
    vec3 normal;
    float uv_y;
    vec4 color;
};

layout(buffer_reference, std430) readonly buffer VertexBuffer{ 
    Vertex vertices[];
};

layout(push_constant) uniform constants {
    mat4 render_matrix; // model matrix
    VertexBuffer vertexBuffer;
} PushConstants;

void main() {
    Vertex v = PushConstants.vertexBuffer.vertices[gl_VertexIndex];

    vec4 worldPos = PushConstants.render_matrix * vec4(v.position, 1.0);
    mat3 normalMat = mat3(transpose(inverse(PushConstants.render_matrix))); // use proper normal matrix in case of non-uniform scale
    vec3 worldN = normalize(normalMat * v.normal);

    vWorldPos = worldPos.xyz;
    vWorldNormal = worldN;
    vUV = vec2(v.uv_x, v.uv_y);
    vVertexColor = v.color.rgb;

    gl_Position = sceneData.viewproj * worldPos;
}
